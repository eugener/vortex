...
fn the_encoding_picker_opens_on_the_one_in_use() {
    ...
    let layer = encoding(&Config::default(), "Shift_JIS");
    assert_eq!(
...
fn an_encoding_the_list_does_not_offer_opens_at_the_top() {
    ...
    let layer = encoding(&Config::default(), "x-mac-cyrillic");
    assert_eq!(
        commit_on_open(layer),
...
fn the_line_ending_picker_offers_both_and_opens_on_the_current_one() {
    let layer = line_ending(&Config::default(), LineEnding::Crlf);
    assert_eq!(
        commit_on_open(layer),
        vec![Command::Editor(Action::SetLineEnding(LineEnding::Crlf))]
    );
    let layer = line_ending(&Config::default(), LineEnding::Lf);
    assert_eq!(
        commit_on_open(layer),
...
fn moving_off_the_current_row_picks_the_other_terminator() {
    let mut layer = line_ending(&Config::default(), LineEnding::Lf);
...
fn neither_picker_previews() {
    ...
    let mut layer = encoding(&Config::default(), "UTF-8");
    send(&mut *layer, press(KeyCode::Down));
...
fn escaping_either_picker_commits_nothing() {
    for mut layer in [
        encoding(&Config::default(), "UTF-8"),
        line_ending(&Config::default(), LineEnding::Lf),
    ] {
